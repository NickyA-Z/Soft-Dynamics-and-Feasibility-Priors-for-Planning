        ...
        for outer_index in range(self.config.outer_steps):
            for inner_index in range(self.config.inner_steps):
                ...
                if (
                    ...
                    print(
                        f"[alm final scale] "
                        f"w_video={float(weighted_video.detach().cpu()):.3f} "
                        f"w_goal={float(weighted_goal.detach().cpu()):.3f} "
                        f"w_action={float(weighted_action.detach().cpu()):.3f} "
                        f"obj={float(objective.detach().cpu()):.3f} "
                        f"rho_pen={float(rho_penalty.detach().cpu()):.3f} "
                        f"dual={float(dual_term.detach().cpu()):.3f} "
                        f"res={float(inner_residual_norm.detach().cpu()):.3f}"
                    )
                ##########################################################
