"""Plan C full-eval, variant 1: rho_max=10.

Same as dino_oracle_full_planc but caps rho at 10 instead of 1000, so
the augmented Lagrangian penalty stays in the same order of magnitude
as the task term (goal/video) and never pushes the iterate off the
outer-1 sweet spot.

Usage:
    python -m src.gvpwm.examples.dino_oracle_full_planc_lowrho \
...
def main():
    ...
    print("[Plan C low-rho] adapter init patched + rho_max=10 (penalty stays "
          "task-scale).")
